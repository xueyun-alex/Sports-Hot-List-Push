...
监控 [今日热榜](https://tophub.today/) **体育**与 **AI** 两大分类，每 5 分钟采集一次，按出现次数生成日报并可选推送到微信。

桌面窗口顶部可切换 **体育 / AI**；下方 **实时热榜 / 历史记录 / 热榜计数** 三个标签页布局一致，数据按当前分类隔离。
...
数据源：[今日热榜 AI 分类首页](https://tophub.today/c/ai?&p=1) 第一页全部模块（约 12 个，随页面变化），每个模块采集 **Top6**。

模块名称与列表在每次抓取时从页面动态发现（以 `hashid` 为稳定键），无需手动维护配置。
...
- `data/hotlist_report.txt` — 体育
- `data/ai_hotlist_report.txt` — AI

若已配置 PushPlus，08:30 / 18:30 会**分两条消息**推送（`体育热榜 | …` 与 `AI热榜 | …`）。
...
建议每天固定时间给 ClawBot 发一条消息保持激活。本项目每天各分类 2 次推送（共 4 条），请注意 ClawBot 每 10 条需主动对话一次的限制。

未设置 `PUSHPLUS_TOKEN` 时仅写入报告文件，不影响采集与报告生成。

### 推送联调

生成测试报告后，试发体育与 AI 各一条推送：

```bash
python verify.py --push
```

需已设置 `PUSHPLUS_TOKEN`。推送测试按钮会按当前选中的分类（体育 / AI）发送对应前缀的测试消息。
...
启动后会打开**桌面监控窗口**，立即采集体育与 AI 各一次，之后每 5 分钟自动刷新。

### 窗口操作

顶部分类栏：**体育** / **AI**（切换后实时热榜、历史、计数均只显示当前分类数据）。

**实时热榜** 标签页：

- **体育**：4 个平台热榜以 2×2 网格排列，Top10
- **AI**：约 12 个模块以可滚动 2 列网格展示，每模块 Top6
- **双击条目**：在浏览器中打开对应链接
...
- `data/records.db`：SQLite 采集记录（`category` 字段区分 `sports` / `ai`）
- `data/hotlist_report.txt`：体育汇总报告
- `data/ai_hotlist_report.txt`：AI 汇总报告
