...
桌面窗口顶部可切换 **体育 / AI**；下方 **实时热榜 / 历史记录 / 热榜计数 / 人物榜单** 四个标签页数据均按当前分类隔离。
...
- **人物排行**：识别热榜标题中的中文姓名、常见体育译名和人物别名，按上榜出现次数降序排列
- **筛选查询**：支持分类、平台、时间范围筛选，并可在人名框中输入关键字查询
- **计数规则**：同一标题内重复出现的同一人只计一次；标题每次被采集到时累计一次
- **别名合并**：常见写法会归一化，例如“勒布朗”与“勒布朗·詹姆斯”计入“詹姆斯”
- **表格列**：排名、人名、平台、出现次数、相关标题数、最近标题、最后出现时间
- **表头排序**：点击“出现次数”或“相关标题”可在降序与升序之间切换，排名会同步更新
- **双击条目**：在浏览器中打开该人物最近一次出现的标题链接（有 URL 时）
